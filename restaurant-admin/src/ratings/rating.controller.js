...
export const createRatingController = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error al guardar la calificación', error: err.message });
    }
};
...
export const getRatingsController = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error al obtener las calificaciones', error: err.message });
    }
};
...
export const deleteRatingController = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error al eliminar la calificación', error: err.message });
    }
};
