...
import { SearchIcon, BuildingIcon, ChevronLeftIcon, ChevronRightIcon } from '../../../shared/components/ui/Icons.jsx';

const CATEGORIES = ['FAST_FOOD', 'FORMAL', 'CAFETERIA', 'ITALIANA', 'MEXICANA', 'MARISCOS', 'PARRILLA', 'VEGANA', 'FUSION', 'DESAYUNOS', 'SUSHI'];
const CAT_LABEL = {
  FAST_FOOD: 'Fast Food', FORMAL: 'Formal', CAFETERIA: 'Cafetería',
  ITALIANA: 'Italiana', MEXICANA: 'Mexicana', MARISCOS: 'Mariscos',
  PARRILLA: 'Parrilla', VEGANA: 'Vegana', FUSION: 'Fusión',
  DESAYUNOS: 'Desayunos', SUSHI: 'Sushi',
};

const EMPTY_FORM = {
  name: '', address: '', phone: '', category: 'FORMAL',
...
export const AdminRestaurantsPage = () => {
  ...
  const load = useCallback(async (p = 1, s = search, cat = catFilter, active = statusFilter) => {
    setLoading(true);
    try {
  ...
  useEffect(() => { load(1); }, [catFilter, statusFilter]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-fp-muted text-sm mt-0.5">
            {total > 0 ? `${total} restaurante${total !== 1 ? 's' : ''} registrados` : 'Sin resultados'}
          </p>
        </div>
        <button
          onClick={openCreate}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-fp-border">
                {['Restaurante', 'Categoría', 'Teléfono', 'Estado', 'Creado', 'Acciones'].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-fp-subtle text-xs uppercase tracking-wide font-medium whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
                ...
                : restaurants.length === 0
                  ? (
                    <tr>
                      <td colSpan={6} className="py-16 text-center">
                        ...
                        <button onClick={openCreate} className="mt-3 text-fp-gold text-sm hover:underline">
                          Crear el primero
                        </button>
                      </td>
                    </tr>
                  ...
                  : restaurants.map((r) => (
                    <tr key={r._id} className="hover:bg-fp-bg/40 transition-colors group">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><StatusBadge active={r.isActive} /></td>
                      <td className="px-4 py-3 text-fp-muted whitespace-nowrap text-xs">{fmt(r.createdAt)}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Nombre *" error={errors.name}>
              <Input value={form.name} onChange={setField('name')} placeholder="El Rincón Gourmet" />
            </Field>
          ...
          <Field label="Dirección *" error={errors.address}>
            <Input value={form.address} onChange={setField('address')} placeholder="Av. Principal 123, Ciudad" />
          </Field>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Teléfono *" error={errors.phone}>
              <Input value={form.phone} onChange={setField('phone')} placeholder="+52 55 0000 0000" />
            </Field>
            <Field label="Email" error={errors.email}>
              <Input value={form.email} onChange={setField('email')} placeholder="contacto@restaurante.com" type="email" />
            </Field>
          </div>

          <Field label="Sitio web" error={errors.website}>
            <Input value={form.website} onChange={setField('website')} placeholder="https://mirestaurante.com" />
          </Field>
