...
import { getUsers, updateUserRole, activateUser, deactivateUser } from '../../../shared/apis/users.js';
...
import { SearchIcon, UsersIcon, ChevronLeftIcon, ChevronRightIcon } from '../../../shared/components/ui/Icons.jsx';
...
export const UsersPage = () => {
  ...
  const fmt = (date) => date ? new Date(date).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-fp-border">
                {['Usuario', 'Email', 'Rol', 'Estado', 'Email verificado', 'Registro', 'Acciones'].map((h) => (
                ...
                : users.length === 0
                  ...
                  : users.map((u) => (
                    <tr key={u.id} className="hover:bg-fp-bg/40 transition-colors group">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap"><StatusBadge active={u.status} /></td>
                      <td className="px-4 py-3 whitespace-nowrap"><EmailBadge verified={u.isEmailVerified} /></td>
                      <td className="px-4 py-3 text-fp-muted whitespace-nowrap text-xs">{fmt(u.createdAt)}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
      ...
      >
        {roleModal.user && (
          <>
            ...
            <ModalActions>
              <BtnSecondary onClick={() => setRoleModal({ open: false, user: null })}>Cancelar</BtnSecondary>
