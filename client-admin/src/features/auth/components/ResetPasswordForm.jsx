...
export const ResetPasswordForm = ({ token, onSwitch }) => {
  ...
  const field =
    'w-full bg-fp-elevated border border-fp-border text-fp-text placeholder-fp-subtle rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-fp-gold focus:ring-1 focus:ring-fp-gold/50 transition-colors';
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-4'>
      <div>
        ...
        />
        {errors.newPassword && (
          <p className='text-fp-danger text-xs mt-1'>{errors.newPassword.message}</p>
        )}
      </div>
      ...
      <div>
        ...
        {errors.confirmPassword && (
          <p className='text-fp-danger text-xs mt-1'>{errors.confirmPassword.message}</p>
        )}
      </div>
