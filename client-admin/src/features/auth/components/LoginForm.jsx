  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        />
        {errors.emailOrUsername && <p className="text-fp-danger text-xs mt-1">{errors.emailOrUsername.message}</p>}
      </div>
      ...
      <div>
        ...
        />
        {errors.password && <p className="text-fp-danger text-xs mt-1">{errors.password.message}</p>}
      </div>
      ...
      <div className="flex flex-col gap-1.5 text-center text-sm pt-1">
        <button type="button" onClick={onForgot} className="text-fp-muted hover:text-fp-gold transition-colors text-xs">
          ¿Olvidaste tu contraseña?
        </button>
        <span className="text-fp-subtle text-xs">
          ¿No tienes cuenta?{' '}
          <button type="button" onClick={onRegister} className="text-fp-gold hover:underline font-medium">
            Regístrate aquí
          </button>
        </span>
      </div>
