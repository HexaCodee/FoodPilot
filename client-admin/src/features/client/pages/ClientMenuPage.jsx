...
const CATEGORY_LABEL = {
  ...
  BEBIDA: 'Bebida', DESAYUNO: 'Desayuno', COMBO: 'Combo',
};
  ...
  <div className="bg-fp-surface border border-fp-border rounded-xl overflow-hidden hover:border-fp-gold/30 transition-colors flex flex-col">
    ...
    <div className="p-4 flex flex-col flex-1">
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${BADGE[item.category] ?? 'bg-fp-elevated text-fp-muted'}`}>
      ...
      <div className="flex items-center justify-between mt-auto pt-2">
        ...
        <div className="flex gap-1 flex-wrap justify-end">
          {item.isVegetarian && <span className="text-xs bg-green-400/10 text-green-400 px-1.5 py-0.5 rounded">Veg</span>}
          {item.isVegan      && <span className="text-xs bg-green-400/10 text-green-500 px-1.5 py-0.5 rounded">Vegano</span>}
          {item.isGlutenFree && <span className="text-xs bg-yellow-400/10 text-yellow-400 px-1.5 py-0.5 rounded">Sin gluten</span>}
        </div>
      </div>
...
export const ClientMenuPage = () => {
  ...
  const loadMenu = useCallback(async () => {
    if (!selectedRestId) { setMenus([]); return; }
    setLoadingMenu(true);
    try {
  ...
  return (
    <div className="space-y-6 animate-fadeUp">
      ...
      <div className="bg-fp-surface border border-fp-border rounded-xl p-4">
        <label className="block text-fp-subtle text-xs mb-2 uppercase tracking-wide">Restaurante</label>
        {loadingRest ? (
          <Skeleton className="h-10 w-full" />
        ) : (
          <select
            value={selectedRestId}
            onChange={(e) => { setSelectedRestId(e.target.value); setSearch(''); setCategory('all'); }}
              ...
              {CATEGORIES.map((c) => (
                <button key={c} onClick={() => setCategory(c)}
                  className={`px-3 py-1 rounded-md text-xs font-medium whitespace-nowrap transition-colors ${
                    category === c ? 'bg-fp-surface text-fp-text shadow-sm' : 'text-fp-muted hover:text-fp-text'
                  }`}>
          ...
          {loadingMenu ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-fp-subtle text-sm mt-1">
                {search || category !== 'all' ? 'Intenta otro filtro' : 'Este restaurante no tiene menú disponible'}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((m) => <MenuCard key={m._id} item={m} />)}
            </div>
          )}

          <p className="text-fp-subtle text-xs text-right">
            {filtered.length} plato{filtered.length !== 1 ? 's' : ''} mostrado{filtered.length !== 1 ? 's' : ''}
          </p>
        </>
      )}
