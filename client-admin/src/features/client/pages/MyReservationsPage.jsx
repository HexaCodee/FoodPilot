...
export const MyReservationsPage = () => {
  ...
  const counts = TABS.reduce((acc, t) => {
    acc[t] = t === 'Todas' ? reservations.length : reservations.filter((r) => r.status === t).length;
    return acc;
  }, {});
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-fp-muted text-sm mt-0.5">
            {loading ? 'Cargando…' : `${reservations.length} reserva${reservations.length !== 1 ? 's' : ''} registrada${reservations.length !== 1 ? 's' : ''}`}
          </p>
        </div>
        <button
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-fp-subtle text-sm mt-1">
              {tab !== 'Todas' ? `No hay reservas ${tab.toLowerCase()}s` : 'Crea tu primera reserva con el botón de arriba'}
            </p>
          </div>
        ) : (
          <div className="divide-y divide-fp-border-subtle">
            {filtered.map((res) => (
              <div key={res._id} className="flex items-center justify-between px-5 py-4 hover:bg-fp-elevated/50 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${STATUS_STYLE[res.status] ?? STATUS_STYLE.COMPLETADA}`}>
