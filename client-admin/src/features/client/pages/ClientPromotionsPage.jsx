...
export const ClientPromotionsPage = () => {
  ...
  useEffect(() => {
    Promise.all([getPromotions({ isActive: true }), getRestaurants({ isActive: true, limit: 50 })])
      .then(([pRes, rRes]) => {
    ...
    <div className='space-y-6 animate-fadeUp'>
      ...
      <div>
        ...
        <p className='text-fp-muted text-sm mt-0.5'>
          {loading
            ? 'Cargando…'
            : `${activeCount} activa${activeCount !== 1 ? 's' : ''} · ${expiredCount} expirada${expiredCount !== 1 ? 's' : ''}`}
        </p>
      </div>
