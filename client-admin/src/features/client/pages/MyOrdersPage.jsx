...
import { getOrders, createOrder } from '../../../shared/apis/orders.js';
...
const TABS = ['Todos', 'PENDIENTE', 'ENVIADO', 'ENTREGADO'];

// ── Create order modal ────────────────────────────────────────────────────────
const CreateOrderModal = ({ isOpen, onClose, onCreated, defaultName }) => {
  const [form, setForm]     = useState({ customerName: '', product: '', quantity: 1 });
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (isOpen) setForm({ customerName: defaultName ?? '', product: '', quantity: 1 });
  }, [isOpen, defaultName]);
