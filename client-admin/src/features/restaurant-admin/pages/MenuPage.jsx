...
const MenuFormModal = ({ isOpen, onClose, onSaved, restaurantId, editing }) => {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={editing ? 'Editar ítem' : 'Nuevo ítem del menú'}
      maxWidth='max-w-xl'
    >
      <div className='space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-4'>
          <Field label='Nombre'>
            <Input
              placeholder='Filete a la plancha'
              value={form.name}
        ...
        <div className='grid grid-cols-2 gap-4'>
          <Field label='Ingredientes (separados por coma)'>
            <Input
              placeholder='Lechuga, tomate, aceite'
    ...
    <div className='space-y-6 animate-fadeUp'>
      <div className='flex items-center justify-between gap-4'>
        <div>
          ...
          <p className='text-fp-muted text-sm mt-0.5'>
            {selectedRestaurant.name} · {items.length} ítem{items.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className='flex flex-col items-center justify-center py-16 text-fp-muted gap-3'>
          ...
          <p className='text-sm'>
            {catFilter !== 'all'
              ? `No hay ítems en "${CAT_LABEL[catFilter]}"`
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <div className='px-5 pt-1 flex items-start justify-between gap-2'>
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${CAT_COLOR[item.category] ?? 'text-fp-muted bg-fp-elevated'}`}
                >
                  {CAT_LABEL[item.category] ?? item.category}
                </span>
                <span className='text-fp-gold font-semibold text-sm'>
              ...
              <div className='flex-1 px-5'>
                ...
                {item.description && (
                  <p className='text-fp-subtle text-xs mt-1 line-clamp-2'>{item.description}</p>
                )}
              </div>
              ...
              <div className='flex flex-wrap gap-1 px-5'>
                ...
                {item.preparationTime && (
                  <span className='text-xs px-2 py-0.5 rounded-full bg-fp-elevated text-fp-muted'>
