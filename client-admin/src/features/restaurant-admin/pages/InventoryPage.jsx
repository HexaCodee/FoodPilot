...
const CATEGORIES = ['BEBIDAS', 'CARNES', 'VERDURAS', 'LACTEOS', 'GRANOS', 'CONDIMENTOS', 'LIMPIEZA', 'OTROS'];
const CAT_LABEL  = { BEBIDAS: 'Bebidas', CARNES: 'Carnes', VERDURAS: 'Verduras', LACTEOS: 'Lácteos', GRANOS: 'Granos', CONDIMENTOS: 'Condimentos', LIMPIEZA: 'Limpieza', OTROS: 'Otros' };
const CAT_BADGE  = {
...
const ItemModal = ({ isOpen, onClose, onSaved, restaurantId, editing }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setForm(editing
        ? { name: editing.name, quantity: String(editing.quantity), unit: editing.unit, minStock: String(editing.minStock ?? 0), category: editing.category }
        : emptyForm
      );
    }
  }, [isOpen, editing]);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={editing ? 'Editar insumo' : 'Nuevo insumo'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input className={field} value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="Ej. Harina de trigo" required />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={label}>Cantidad *</label>
            <input type="number" min="0" step="0.01" className={field} value={form.quantity} onChange={(e) => set('quantity', e.target.value)} required />
          </div>
          <div>
            <label className={label}>Unidad *</label>
            <input className={field} value={form.unit} onChange={(e) => set('unit', e.target.value)} placeholder="kg, L, pzas…" required />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={label}>Stock mínimo</label>
            <input type="number" min="0" step="0.01" className={field} value={form.minStock} onChange={(e) => set('minStock', e.target.value)} />
          </div>
          <div>
        ...
        <ModalActions>
          ...
          <BtnPrimary type="submit" loading={saving}>{editing ? 'Guardar' : 'Crear'}</BtnPrimary>
        </ModalActions>
      </form>
    </Modal>
  );
...
const DeleteModal = ({ item, onClose, onDeleted }) => {
  ...
  return (
    <Modal isOpen={!!item} onClose={onClose} title="Eliminar insumo">
      <p className="text-fp-muted text-sm mb-2">¿Eliminar el insumo <strong className="text-fp-text">{item?.name}</strong>?</p>
      <ModalActions>
    ...
    <div className="space-y-5 animate-fadeUp">
        ...
        {['all', ...CATEGORIES].map((c) => (
          <button key={c} onClick={() => setCatFilter(c)}
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
              catFilter === c ? 'bg-fp-surface text-fp-text shadow-sm' : 'text-fp-muted hover:text-fp-text'
            }`}>
      ...
      <div className="bg-fp-surface border border-fp-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-fp-border flex items-center justify-between">
          ...
          <span className="text-fp-subtle text-xs">{loading ? '…' : `${filtered.length} registros`}</span>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-fp-subtle text-sm text-center py-10">Sin insumos{catFilter !== 'all' ? ' en esta categoría' : ''}</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-fp-border bg-fp-bg">
                  {['Nombre', 'Cantidad', 'Unidad', 'Stock mín.', 'Categoría', 'Acciones'].map((h) => (
                ...
                {filtered.map((item) => {
                  ...
                  return (
                    <tr key={item._id} className="hover:bg-fp-elevated/50 transition-colors">
                      ...
                      <td className={`px-4 py-3 font-semibold ${lowStockFlag ? 'text-red-400' : 'text-fp-text'}`}>
                        {item.quantity}
                      </td>
                      <td className="px-4 py-3 text-fp-muted">{item.unit}</td>
                      <td className="px-4 py-3 text-fp-muted">{item.minStock ?? 0}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CAT_BADGE[item.category] ?? CAT_BADGE.OTROS}`}>
