...
import { getTables, createTable, updateTable, activateTable, deactivateTable } from '../../../shared/apis/tables.js';
...
const TableFormModal = ({ isOpen, onClose, onSaved, restaurantId, editing }) => {
  ...
  useEffect(() => {
    if (!isOpen) return;
    setForm(editing
      ? { tableNumber: editing.tableNumber, capacity: editing.capacity, location: editing.location, status: editing.status }
      : EMPTY
    );
    setError('');
  }, [isOpen, editing]);
  ...
  const handleSubmit = async () => {
    if (!form.tableNumber || !form.capacity) { setError('Número de mesa y capacidad son requeridos.'); return; }
    setSaving(true); setError('');
    try {
      const payload = { tableNumber: Number(form.tableNumber), capacity: Number(form.capacity), location: form.location, status: form.status, restaurant: restaurantId };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editing ? 'Editar mesa' : 'Nueva mesa'}>
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label="N° de mesa">
            <Input type="number" min="1" placeholder="1" value={form.tableNumber} onChange={(e) => set('tableNumber', e.target.value)} />
          </Field>
          <Field label="Capacidad">
            <Input type="number" min="1" placeholder="4" value={form.capacity} onChange={(e) => set('capacity', e.target.value)} />
          </Field>
        </div>
      ...
      <ModalActions>
        ...
        <BtnPrimary onClick={handleSubmit} loading={saving}>{editing ? 'Guardar cambios' : 'Crear mesa'}</BtnPrimary>
      </ModalActions>
    </Modal>
  );
};
...
export const TablesPage = () => {
  ...
  const handleToggle = async (t) => {
    ...
    try {
      ...
      fetchTables();
    } finally { setBusy((p) => ({ ...p, [t._id]: false })); }
  };
    ...
    <div className="space-y-6 animate-fadeUp">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-fp-muted text-sm mt-0.5">{selectedRestaurant.name} · {tables.length} mesa{tables.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-fp-muted gap-3">
          <TableIcon className="w-10 h-10 opacity-30" />
          <p className="text-sm">{filter !== 'all' ? `No hay mesas con estado "${STATUS_LABEL[filter]}"` : 'No hay mesas registradas'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((t) => (
            <div key={t._id} className="bg-fp-surface border border-fp-border rounded-xl p-5 flex flex-col gap-4 hover:border-fp-gold/20 transition-colors">
              <div className="flex items-start justify-between">
                <div className="p-2.5 rounded-lg bg-fp-gold-dim">
                  <TableIcon className="w-5 h-5 text-fp-gold" />
                </div>
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_COLOR[t.status] ?? STATUS_COLOR.FUERA_DE_SERVICIO}`}>
                  {STATUS_LABEL[t.status] ?? t.status}
                </span>
              </div>
              <div>
                <p className="text-fp-text font-semibold text-lg">Mesa {t.tableNumber}</p>
                <p className="text-fp-muted text-sm mt-0.5">{t.capacity} personas · {t.location}</p>
              </div>
              <div className="flex gap-2 mt-auto pt-2 border-t border-fp-border-subtle">
                ...
                <button onClick={() => handleToggle(t)} disabled={!!busy[t._id]}
      ...
      <TableFormModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSaved={fetchTables}
