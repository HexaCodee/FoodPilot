    ...
    <Modal isOpen={!!item} onClose={onClose} title={isEdit ? 'Editar promoción' : 'Nueva promoción'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input className={f} value={form.title}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className={f} value={form.code}
              onChange={(e) => set('code', e.target.value.toUpperCase())} maxLength={20}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="date" className={f} value={form.validFrom}
              onChange={(e) => set('validFrom', e.target.value)} />
          </div>
          <div>
            <label className={l}>Válida hasta</label>
            <input type="date" className={f} value={form.validTo}
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-fp-muted text-sm mt-0.5">
            {selectedRestaurant.name} · {loading ? 'Cargando…' : `${promotions.length} promoción${promotions.length !== 1 ? 'es' : ''}`}
          </p>
        </div>
        <button
            ...
            {promotions.map((p) => {
              ...
              return (
                <div key={p._id}
                  ...
                  <div>
                    <p className={`text-sm font-medium truncate ${expired ? 'text-fp-muted' : 'text-fp-text'}`}>
                      {p.title}
                    </p>
                    {p.description && (
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium w-fit ${
                    !p.isActive     ? 'bg-fp-elevated text-fp-muted'
