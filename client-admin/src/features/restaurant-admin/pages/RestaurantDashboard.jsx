...
import {
  TableIcon, UtensilsIcon, BagIcon, CalendarIcon,
  BarChartIcon, BuildingIcon, CheckIcon,
...
const OrderRow = ({ order }) => (
  <div className="flex items-center justify-between py-3 border-b border-fp-border-subtle last:border-0">
    ...
    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_COLOR[order.status] ?? STATUS_COLOR.PENDIENTE}`}>
            ...
            {restaurants.map((r) => (
              ...
              >
                <div>
                  ...
                  <p className="text-fp-subtle text-xs mt-0.5">{r.category} · {r.address}</p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${r.isActive ? 'bg-fp-success-dim text-fp-success' : 'bg-fp-elevated text-fp-muted'}`}>
...
export const RestaurantDashboard = () => {
  ...
  const [stats, setStats]     = useState({ tables: null, menus: null, orders: null, reservations: null });
    ...
    <div className="space-y-6 animate-fadeUp">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-fp-muted text-sm mt-0.5">{selectedRestaurant.category} · {selectedRestaurant.address}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard Icon={TableIcon}    label="Mesas disponibles" value={stats.tables}       loading={loading} />
        <StatCard Icon={UtensilsIcon} label="Ítems en menú"     value={stats.menus}        loading={loading} />
        <StatCard Icon={BagIcon}      label="Pedidos pendientes" value={stats.orders}      loading={loading} />
        <StatCard Icon={CalendarIcon} label="Reservas activas"  value={stats.reservations} loading={loading} />
      </div>
